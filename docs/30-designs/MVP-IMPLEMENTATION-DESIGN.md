...
3. Codex Hook 捕获文件名、before 内容和 after 内容。
4. Hook 通过本地 HTTP 通知 Plugin。
5. Plugin 将变动集交给 Governance Server。
6. Governance Server 更新 Incremental Workspace Index。
7. Governance Server 调用 `.pickles.json` 中的 ArchUnit / ESLint 命令。
8. Governance Server 聚合 Problem。
9. Problem Board 展示 `title`、`type`、`message`。
10. Hook 在 task 完成前请求治理反馈。

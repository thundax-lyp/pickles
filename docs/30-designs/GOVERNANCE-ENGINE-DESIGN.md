...
- Runtime 不拥有 UI。
- Runtime 不修改业务代码。
- Runtime 直接调用 `.pickles.json` 中配置的用户工程命令。
...
Runtime 从目标工程根目录读取 `.pickles.json`。
...
Runtime 根据 `.pickles.json` 执行用户工程命令：
...
- Runtime 不写 `.pickles.json`，配置写入由 Plugin 负责。

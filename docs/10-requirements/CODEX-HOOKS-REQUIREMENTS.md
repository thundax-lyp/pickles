...
Hook 必须在 `PreToolUse` 观察即将执行的 `apply_patch` 或 shell 操作，并记录 before 内容或变动线索。
...
2. Hook 记录 before 内容或变动线索。
3. Codex task 触发 `PostToolUse`。
4. Hook 捕获文件名、before 内容和 after 内容。
5. Hook 通过本地 HTTP 通知 IntelliJ Plugin。
6. Plugin / Governance Engine 更新 Problem Board。

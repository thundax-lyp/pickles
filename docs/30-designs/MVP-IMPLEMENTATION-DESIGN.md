...
- Hook 事件在 MVP 固定为 `SessionStart`、`PostToolUse` 和 `Stop`。
...
4. Codex Hook 在 `PostToolUse` 捕获文件名、before 内容和 after 内容。
5. Hook 通过本地 HTTP 通知 Plugin。
6. Plugin 将变动集交给 Governance Server。
7. Governance Server 更新 Incremental Workspace Index。
8. Governance Server 调用 `.pickles.json` 中的 ArchUnit / ESLint 命令。
9. Governance Server 聚合 Problem。
10. Problem Board 展示 `title`、`type`、`message`。
11. Hook 在 `Stop` 请求治理反馈。
...
1. Hook 在 `PostToolUse` 提交文件变动。
2. Runtime 更新 workspace index。
3. Runtime 执行规则命令。
4. Runtime 聚合 Problem。
5. Plugin 刷新 Problem Board。

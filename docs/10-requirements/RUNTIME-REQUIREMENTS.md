...
`extends / implements` 只表示源码中的声明文本，不表示跨文件类型解析、继承闭包或 classpath 语义。
...
Parser diagnostic message 必须稳定可读，不得暴露 raw tree-sitter node dump。

Runtime 必须对单文件 parser diagnostic 去重并限制数量，初始上限固定为 20 条。
...
- Runtime parser 测试必须覆盖 import 变体、泛型声明、字段与注解变体、继承声明和 diagnostic 上限。

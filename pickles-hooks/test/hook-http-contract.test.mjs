...
test("hook captures modified added and deleted files", async () => {
  ...
  try {
    ...
    const statePath = path.join(repo, ".pickles", "hooks-state", "session-capture", "turn-capture", "tool-capture.json");
    ...
    assert.equal(notify.body.files.every((file) => path.isAbsolute(file.fileName) === false), true);
  } finally {
    await close(server);
  }
});
...
test("hook falls back to git HEAD when PostToolUse state is missing", async () => {
  ...
  try {
    ...
    const notify = requests.find((request) => request.method === "POST" && request.url === "/notify");
    assert.deepEqual(notify.body.files, [{ fileName: "fallback.txt", before: "old\n", after: "new\n" }]);
  } finally {
    await close(server);
  }
});
...
test("hook flushes pending state before feedback on Stop", async () => {
  ...
  try {
    ...
    assert.equal(pre.status, 0, pre.stderr);
    const statePath = path.join(repo, ".pickles", "hooks-state", "session-stop", "turn-stop", "tool-stop.json");
    assert.equal(existsSync(statePath), true);
    ...
    const notifyIndex = requests.findIndex((request) => request.method === "POST" && request.url === "/notify");
    const feedbackIndex = requests.findIndex((request) => request.method === "POST" && request.url === "/feedback");
    assert.equal(notifyIndex >= 0, true);
    assert.equal(feedbackIndex > notifyIndex, true);
    assert.deepEqual(requests[notifyIndex].body.files, [{ fileName: "pending.txt", before: "old\n", after: "new\n" }]);

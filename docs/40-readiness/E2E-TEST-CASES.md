...
8. Runtime 读取 `<repo>/pickles.config.ts` 并执行规则命令。
...
- `e2e/sample-project/`: 目标工程样例、Pickles runtime config、TypeScript 规则命令。
...
- TypeScript 类型检查通过。
- lint 命令通过。
- Pickles runtime config 中 `adapter: "eslint"` 规则可指向样例工程验证命令。

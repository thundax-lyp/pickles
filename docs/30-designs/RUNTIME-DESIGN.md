...
`extendsTypes` 和 `implementsTypes` 只保存声明文本，不做跨文件符号解析。
...
Diagnostic message 固定使用稳定可读文本。

单文件 diagnostic 必须去重并限制数量，初始上限固定为 20 条。
...
- nested type。
- annotated method。
- import declaration 变体。
- generic class 和 generic method。
- field declaration 与 annotation 变体。
- class extends / implements。
- interface extends。
- record implements。
- parser diagnostic 去重和 20 条上限。
- constructor。
- syntax error。

...
- `.pickles/config.json` 固定放在目标工程 `.pickles/` 目录。
...
1. Plugin 打开目标工程。
2. Plugin 读取目标工程 `.pickles/config.json`。
...
11. Governance Server 调用 `.pickles/config.json` 中的 ArchUnit / ESLint 命令。

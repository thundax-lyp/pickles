...
8. Runtime 读取 `<repo>/.pickles/config.json` 并执行规则命令。
...
- `pickles-runtime/`: Governance Runtime、规则命令执行、Problem 聚合。
- `e2e/sample-project/`: 目标工程样例、`.pickles/config.json`、TypeScript 规则命令。
...
- TypeScript 类型检查通过。
- lint 命令通过。
- `.pickles/config.json` 中 `rules.eslint.command` 可指向样例工程验证命令。
...
目标：验证 `pickles-runtime/` 能基于 changed files 和 `.pickles/config.json` 执行样例工程规则命令并生成 Problem。
...
- Runtime 读取 `<repo>/.pickles/config.json`。
- Runtime 基于 changed files 更新 workspace index。
- Runtime 执行 `rules.eslint.command`。

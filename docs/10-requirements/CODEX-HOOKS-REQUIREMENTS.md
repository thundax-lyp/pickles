...
3. Hook 读取目标工程 `.pickles/server.json`。
4. Hook 使用 `http://127.0.0.1:<port>` 检查本地 Plugin HTTP 服务是否可用。
5. Hook 将初始化结果暴露给 Codex。
...
- Hook 通信失败不得修改用户工程文件。
- Hook 不保存全量语义持久化数据。
- Hook 重试或重复通知不得导致同一 `session_id` / `turn_id` 的同一文件变动被重复应用。
